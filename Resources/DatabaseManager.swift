...
extension DatabaseManager {
    ...
    public func conversationExist(with targetRecipientEmail: String, completion: @escaping (Result<String, Error>) -> Void) {
        let safeRecipmentEmail = DatabaseManager.safeEmail(emailAddress: targetRecipientEmail)
        guard let senderEmail = UserDefaults.standard.value(forKey: "email") as? String else { return }
        let safeSenderEmail = DatabaseManager.safeEmail(emailAddress: senderEmail)
        database.child("\(safeRecipmentEmail)/conversations").observeSingleEvent(of: .value, with: { snapshot in
    ...
    public func getAllConversations(for email: String, completion: @escaping (Result<[Conversation], Error>) -> Void) {
        database.child("\(email)/conversations").observe(.value, with: { snapshot in
            ...
            let conversations: [Conversation] = snapshotValue.compactMap({ dictionary in
                guard let dictionID = dictionary["id"] as? String,
                ...
                return Conversation(id: dictionID, name: name, otherUserEmail: otherUserEmail, latestMessage: latestMessageObject)
            })
    ...
    public func  createNewConversation(with otherUserEmail: String, name: String, firstMessage: Message, completion: @escaping (Bool) -> Void) {
        ...
        ref.observeSingleEvent(of: .value, with: { [weak self] snapshot in
            ...
            let dateString = OneChatVC.dateFormatter.string(from: messageDate)
        ...
        database.child("\(id)/messages").observe(.value, with: { snapshot in
            ...
            let messages: [Message] = value.compactMap({ dictionary in
                guard let name = dictionary["name"] as? String,
                    ...
                    let date = OneChatVC.dateFormatter.date(from: dateString)else {
                        return nil
                }
